...
export class Transport {
  ...
  async response(path: string, options: InternalRequestOptions = {}): Promise<Response> {
    const outcome = await this.fetchWithAuthRetry(path, options)
    if (outcome.response.ok || outcome.response.status === 304)
  ...
  private async fetchOnce(path: string, options: InternalRequestOptions): Promise<FetchOutcome> {
    ...
    const headers = await this.requestHeaders(options.headers, request)
    const accessToken = await this.authorizeRequest(headers, options.auth !== false, request)
    const body = this.serializeBody(options.body, headers, request)
  ...
  private serializeBody(
    ...
    try {
      return this.prepareBody(body, headers)
    }
    catch (cause) {
  ...
  private envelopeError<T>(
    ...
    if (envelope.error)
      return HertaError.api(envelope.error, request)
