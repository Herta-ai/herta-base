...
export class WebProjectsAdminClient {
  ...
  async deploy(input: WebProjectDeploy): Promise<WebProject> {
    ...
    if (input.spaFallback !== undefined)
      form.append('spaFallback', String(input.spaFallback))
    if (input.cacheControl !== undefined)
      form.append('cacheControl', input.cacheControl)
    if (input.notFound !== undefined)
      form.append('notFound', input.notFound ?? '')
